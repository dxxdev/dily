...
const OriginalCategoryProducts = ({ one, setone }) => {
  ...
  useEffect(() => {
    const productsInCat = filteredOriginalCategory(products, pathname);
  ...
  const brandsFilter = availableFilters.find(f => f.name === "Brend");
  const brends = brandsFilter ? brandsFilter.values : [];
          ...
          <ul className="flex justify-start items-center gap-4">
            {brends &&
              ...
              brends.map((brend, index) => {
                return (
                  <button key={index} onClick={() => handleFilter("Brend", brend)}>
                    <li className={`py-3 px-4 rounded-[10px] shadow-product-card-shadow ${
                      activeFilter.name === "Brend" && activeFilter.value === brend ? "bg-bright-green text-white" : "bg-white text-dark-gray"
                    }`}>
                      {brend}
                    </li>
                  </button>
