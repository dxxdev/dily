...
import { emptySaved } from "../assets/icons";
const Save = ({ one, setone }) => {
  const filtered = products.filter((item) => item.saved === true);
  const filteredByCategory = filterOriginalCategory(filtered);
  const [viewType, setViewType] = useState("grid");
  console.log(filteredByCategory);
  return (
    <div className="bg-light-gray">
      <div className="my-container flex flex-col items-start gap-20 justify-between">
        <div className="w-full flex flex-col items-end justify-between gap-2 space-y-7">
          <div className="w-full flex justify-between items-end">
            <h2 className="text-3xl font-bold leading-120 text-dark-gray">
              Saqlanganlar
            </h2>
            <div className="flex justify-center items-center gap-2.5">
              <span className="font-open-sans text-sm tracking-wider first-letter:uppercase lowercase">
                Ro'yhat Turini O'zgartirish
              </span>
              {/* change view grid */}
              <button
                onClick={() => {
                ...
                disabled={filtered && filtered.length > 0 ? false : true}
              >
                {viewType == "grid" ? (
                  <svg
                    width="18"
          ...
          {filteredByCategory && filteredByCategory.length > 0 ? (
            <ul
              className={`grid ${
                viewType !== "grid" ? "grid-cols-1" : "grid-cols-5"
              } gap-5 w-full`}
            >
            ...
            <div className="my-container flex flex-col items-center justify-center h-96">
              <div className="w-max flex flex-col justify-center items-center gap-6">
                <p className="text-dark-gray font-medium leading-normal text-lg">
                  Saqlanganlar bo'sh
                </p>
                <p className="text-dark-gray font-medium leading-normal text-lg">
                  Mahsulotni keyinroq ko'rish yoki sotib olish uchun
                  saqlanganlarga qo'shing
                </p>
                <img src={emptySaved} alt="" />
              </div>
            </div>
          )}
        </div>
